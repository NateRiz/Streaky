...
class InstructionLibrary{
  ...
  private:
    ...
    if(cfg.DECAY_TEST() != "NO_REGULATOR"){
    ...
    inst_lib.AddInst("Inst_ExtRegulator", CH::hardware_t::Inst_ExtRegulator, 1, "Extend regulator duration by Arg1");
    ...
    /*inst_lib.AddInst(
        ...
        [&](typename CH::hardware_t& hw, const typename CH::inst_t& inst){
          typename CH::state_t& state = hw.GetCurState(); 
          std::cout <<"["<<inst.args[0]<<"] : "<< state.GetLocal(inst.args[0]) <<std::endl;
        },
        1,
        "debug");
        */
